...
def run_pipeline(
    ...
    try:
        ...
        if existing_desc and resume:
            ...
            if verbose:
                print(f"\n{'='*60}")
                print(f"[Stage 1/3] 图片分析 (img2text)")
                print(f"{'='*60}")
            ...
            analysis_result = analyzer.analyze(str(image_path))
...
示例:
  # 单张图片处理
  python pipeline.py -i screenshot.jpg

  # 指定输出目录
  python pipeline.py -i screenshot.jpg -o ./output

  # 批量处理目录
  python pipeline.py --input-dir ./screenshots

  # 生成对比图
  python pipeline.py -i screenshot.jpg --compare
