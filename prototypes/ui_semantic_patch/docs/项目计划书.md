...
**目标**: 系统性扩展异常模式库，从当前已实现模式继续增加覆盖面

> **核心问题**: 已有模式仍难覆盖真实场景异常多样性，尤其是细粒度文字编辑在检测分组不稳定时效果波动。
...
| 5 | modify_text_ai / modify_text_ocr / modify_text | `renderers/text_overlay.py` | 细粒度文字替换（AI 或 OCR+PIL） |
| 6 | modify_text_e2e | `renderers/text_overlay.py` | 跳过检测分组的端到端文字编辑 |
...
| **M2: 多模式渲染** | 基础四种 + 文字编辑（`modify_text*`）可用 | ✅ 已达成 | Phase 2 中期 |
...
- **多种异常模式**: dialog、area_loading、content_duplicate、text_overlay、modify_text_ai、modify_text_ocr（含 modify_text）、modify_text_e2e
...
**创建日期**: 2026-01-09
**最后更新**: 2026-03-26
**文档同步**: 可执行命令与环境变量以仓库根目录 [Claude.md](../../../Claude.md) 为准。
**版本**: v2.0
