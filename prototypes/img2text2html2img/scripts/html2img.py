...
def extract_resolution_from_html(html_content: str) -> Optional[Tuple[int, int]]:
    ...
    patterns = [
        ...
        # html, body { width: 1080px; height: 2340px; }
        r'html\s*,\s*body\s*\{[^}]*width:\s*(\d+)px[^}]*height:\s*(\d+)px',
        r'html\s*,\s*body\s*\{[^}]*height:\s*(\d+)px[^}]*width:\s*(\d+)px',
        r'body\s*\{[^}]*width:\s*(\d+)px[^}]*height:\s*(\d+)px',
        r'body\s*\{[^}]*height:\s*(\d+)px[^}]*width:\s*(\d+)px',
        # 通用: width:1080px;height:2340px (连续出现)
        r'width:\s*(\d+)px;\s*height:\s*(\d+)px',
        r'height:\s*(\d+)px;\s*width:\s*(\d+)px',
    ]
    for p in patterns:
        ...
        if m:
            groups = m.groups()
            # 判断哪个是width哪个是height
            if 'height' in p and p.index('height') < p.index('width'):
