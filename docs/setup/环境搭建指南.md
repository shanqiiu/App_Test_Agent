...
以下命令默认在**仓库根目录**执行（即 `App_Test_Agent/`）：
...
打开 `.env` 文件并填入实际的 API 密钥（与根目录 `Claude.md` 一致）：

```bash
# 必需：VLM（OpenAI 兼容接口，用于 UI 分析 / 语义理解）
VLM_API_KEY=sk-xxxxxxxxxxxxxxxxxxxx
VLM_API_URL=https://api.openai-next.com/v1/chat/completions

# 可选：DashScope（AI 图像生成 / 编辑）
...
```bash
cd ui_semantic_patch/third_party/OmniParser
```
...
```bash
python -c "import requests, PIL; print('python deps ok')"
```
...
# 弹窗模式（需已配置 VLM_API_KEY，dialog 默认）
python run_pipeline.py \
  --screenshot ../data/原图/app首页类-开屏广告弹窗/携程旅行01.jpg \
  --instruction "生成优惠券广告弹窗" \
  --gt-category "弹窗覆盖原UI" \
  --gt-sample "弹出广告.jpg" \
  --output ./output/env-check
```

**预期**：控制台打印 Stage 1–3 日志，并在 `output/env-check` 下生成 `*_final_*.png` 等产物。

（文字编辑示例见根目录 `Claude.md`：`modify_text_ai` + GT `弹窗覆盖原UI/05.jpg`。）
...
```bash
# 检查模型文件是否存在
ls ui_semantic_patch/third_party/OmniParser/weights/
...
**解决**:
1. 确认 `.env` 文件在项目根目录
2. 检查是否正确填写 `VLM_API_KEY=`（及可选 `DASHSCOPE_API_KEY=`）
...
1. **阅读项目文档**: [docs/](../../docs/)
2. **查看示例**: [ui_semantic_patch/examples/](../../ui_semantic_patch/examples/)
...
| 2026-03-26 | v1.1 | `.env` 示例改为 `VLM_API_KEY`；ui_semantic_patch 验证命令与当前流水线对齐 |

---

**最后更新**: 2026-03-26
**文档同步**: `.env` 变量名与验证命令与 [Claude.md](../../Claude.md) 对齐。
