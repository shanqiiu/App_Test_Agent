...
def generate_meta_for_directory(
    ...
    # 8. 提取弹窗精确边界（可选，需要 OmniParser）
    if extract_bounds and category_id == 'dialog_blocking' and not dry_run:
        print(f"\n{'='*60}")
        print(f"提取弹窗边界框 (dialog_bounds_px)")
        print(f"{'='*60}")
        try:
            from analysis.gt_bounds import extract_all_bounds
            bounds_results = extract_all_bounds(
            ...
            for name, bounds in bounds_results.items():
                meta['samples'][name]['dialog_bounds_px'] = bounds
            print(f"\n  ✓ 提取完成: {len(bounds_results)} 个样本")
        except Exception as e:
            print(f"\n  ⚠ 边界框提取失败 (OmniParser 可能不可用): {e}")
            print(f"  可稍后单独运行: python -m analysis.gt_bounds --category {target_path.name}")
...
"""
    ...
    parser.add_argument('--dry-run', action='store_true',
                        help='仅预览，不写入（默认行为）')
    parser.add_argument('--extract-bounds', action='store_true',
                        help='同时提取 dialog_bounds_px（需要 OmniParser）')
    parser.add_argument('--omni-device', default=None,
                        help='OmniParser 运行设备（与 --extract-bounds 配合使用）')
    ...
    common_kwargs = dict(
        ...
        extract_bounds=args.extract_bounds,
