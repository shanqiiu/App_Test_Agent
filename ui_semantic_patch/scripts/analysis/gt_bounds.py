    ...
    for i in range(0, len(croped_pil_image), batch_size):
        ...
        if 'florence' in model.config.name_or_path:
            generated_ids = model.generate(input_ids=inputs["input_ids"], pixel_values=inputs["pixel_values"],
                                           max_new_tokens=20, num_beams=1, do_sample=False, use_cache=False)
        else:
    ...
    for i, box1 in enumerate(boxes):
        ...
        if is_valid_box:
            ...
            if ocr_bbox:
                ...
                if not any(IoU(box1, box3) > iou_threshold and not is_inside(box1, box3) for k, box3 in
                           enumerate(ocr_bbox)):
...
def check_ocr_box(image_source: Union[str, Image.Image], display_img=False, output_bb_format='xywh',
