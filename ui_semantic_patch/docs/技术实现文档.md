...
图像生成后端不再通过命令行参数切换，而是通过 `.env` 中的 `IMAGE_GEN_BACKEND` 控制：

| 值 | 说明 | 行为 |
|-----|------|------|
| `dashscope` | 强制使用 DashScope API | 本地开发环境推荐 |
| `local` | 强制使用本地文生图服务 | 服务器环境推荐 |
| `auto` | 自动选择 | 优先本地服务，未配置 `LOCAL_IMAGE_API_URL` 时回退到 DashScope |

DashScope 模型也支持单独配置：

| 环境变量 | 默认值 | 用途 |
|---------|--------|------|
| `DASHSCOPE_IMAGE_GEN_MODEL` | `qwen-image-max` | 纯文生图 |
| `DASHSCOPE_IMAGE_EDIT_MODEL` | `qwen-image-edit-max` | 带参考图的图像编辑 |
...
IMAGE_GEN_BACKEND=dashscope  # 图像生成后端：dashscope / local / auto
DASHSCOPE_IMAGE_GEN_MODEL=qwen-image-max
DASHSCOPE_IMAGE_EDIT_MODEL=qwen-image-edit-max
LOCAL_IMAGE_API_URL=         # 本地文生图服务地址（服务器环境可配置）
...
| 方式 | 实现 | 适用场景 | 后端说明 |
|------|------|----------|---------|
| PIL 绘制 | `generate_dialog_pil()` | 简单弹窗、文字清晰度要求高 | 不依赖图像生成服务 |
| DashScope 文生图 | `generate_image_dashscope()` + `DASHSCOPE_IMAGE_GEN_MODEL` | 复杂视觉效果、图片内容 | `IMAGE_GEN_BACKEND=dashscope` 或 `auto` 回退 |
| DashScope 图像编辑 | `generate_image_dashscope()` + `DASHSCOPE_IMAGE_EDIT_MODEL` | 基于参考图编辑生成 | 带参考图路径时优先使用 |
| 本地文生图服务 | `generate_image_local()` | 服务器侧已部署内部文生图服务 | `IMAGE_GEN_BACKEND=local` |
| 统一入口 | `generate_image()` | 根据环境自动路由 | 推荐主路径使用 |
...
│   ├── semantic_dialog_generator.py  # 弹窗生成核心（DashScope / 本地服务 / PIL）
...
| 脚本 | 用途 | 依赖 |
|------|------|------|
| `run_pipeline.py` | 单图异常生成主流水线 | OmniParser + VLM + 图像生成后端（DashScope / 本地服务） |
...
**最后更新**: 2026-03-26
**文档同步**: 流水线参数与示例命令以仓库根目录 [Claude.md](../../../Claude.md) 为准。
