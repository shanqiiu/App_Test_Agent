...
用法:
  # 单目录生成（默认 dry-run 预览）
  python generate_meta.py --dir "../data/Agent 执行遇到的典型异常 UI 类型/analysis/gt_templates/弹窗覆盖原 UI"

  # 实际写入
  python generate_meta.py --dir "../data/.../弹窗覆盖原 UI" --run

  # 覆盖已有 meta.json
  python generate_meta.py --dir "../data/.../弹窗覆盖原 UI" --run --force

  # 批量扫描所有子目录
  python generate_meta.py --scan-all "../data/Agent 执行遇到的典型异常 UI 类型/analysis/gt_templates" --run
...
# 初始化路径（添加项目目录到 sys.path）
init_app_paths(include_utils=True)

# 从环境变量读取配置
VLM_API_KEY = os.environ.get('VLM_API_KEY')
VLM_API_URL = config.VLM_API_URL  # 使用集中配置
...
from app.generators.prompts import (
    PROMPT_DIALOG_BLOCKING,
    PROMPT_CONTENT_DUPLICATE,
    PROMPT_LOADING_TIMEOUT,
    CATEGORY_TO_PROMPT,
)
...
# ============================================================
DIR_TO_CATEGORY = {
    '弹窗覆盖原UI': ('dialog_blocking', '弹窗覆盖 UI - 用于生成各类遮挡弹窗异常'),
    '弹窗覆盖原 UI': ('dialog_blocking', '弹窗覆盖 UI - 用于生成各类遮挡弹窗异常'),
    '内容歧义、重复': ('content_duplicate', '内容歧义/重复 - UI 元素重复显示导致操作歧义'),
...
def generate_meta_for_directory(
    ...
    print(f"\n{'='*60}")
    print(f"generate_meta.py - GT 模板元数据自动生成")
    print(f"{'='*60}")
    print(f"  目录：      {target_dir}")
    print(f"  类别 ID:     {category_id}")
    print(f"  描述：      {description}")
    print(f"  图片数：    {len(images)} 张")
    if existing_meta:
        print(f"  已有 meta:   是 ({len(existing_samples)} 个样本)")
    else:
        print(f"  已有 meta:   否")
...
def scan_all_directories(
    ...
    **kwargs,
    ...
    for d in subdirs:
        ...
        print(f"  {'[有 meta]' if has_meta else '[无 meta]'} {d.name} ({img_count} 张)")
...
def main():
    parser = argparse.ArgumentParser(
        description='GT 模板 meta.json 自动生成（VLM 驱动）',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
  # 预览（默认 dry-run）
  python generate_meta.py --dir "../data/.../gt_templates/弹窗覆盖原 UI"

  # 实际写入
  python generate_meta.py --dir "../data/.../gt_templates/弹窗覆盖原 UI" --run

  # 覆盖已有 meta.json
  python generate_meta.py --dir "../data/.../gt_templates/弹窗覆盖原 UI" --run --force
