...
## 任务
第一步：判断这个裁剪区域是否需要编辑。
- 描述这个区域在原图中的功能角色（如：车次卡片中的席位票量显示、筛选按钮、商品标题、价格标签、促销横幅、导航Tab、列表项、状态文字等）
- 判断编辑这个区域是否真的符合指令意图
- 如果区域的功能角色与指令的编辑目标不匹配（如指令要求修改车次卡片内的票量显示、但这里是页面顶部的筛选按钮），则不应该编辑
...
如果**不需要编辑**，输出：
```json
{{"relevant": false, "skip_reason": "<简短说明，如：这是页面顶部的筛选按钮，而指令要求修改车次卡片内的票量信息>"}}
```
...
{{
  "relevant": true,
  "component_role": "<该区域的功能角色，如：车次卡片中的席位票量显示>",
  "edits": [
    {{
      "action": "modify_text",
      "region": {{
