...
def generate_image_dashscope(
    ...
    model, use_reference = _resolve_dashscope_models(force_model, bool(has_ref))

    if not use_reference:
        messages = [{"role": "user", "content": [{"text": prompt}]}]
        if force_model == 'gen' and has_ref:
            print(f"  ℹ 强制文生图模式，忽略参考图 (model={model})")
        elif force_model == 'edit' and has_ref:
            print(f"  ⚠ 指定 edit 模式但无法使用参考图，回退到文生图 (model={model})")
        else:
...
def generate_image(
    ...
    if backend == "dashscope":
        print("  ℹ 图像生成后端: dashscope")
        return generate_image_dashscope(
            prompt=prompt,
            api_key=api_key,
            size=size,
            negative_prompt=negative_prompt,
    ...
    local_api_url = os.getenv("LOCAL_IMAGE_API_URL")
    if local_api_url:
        print("  ℹ 图像生成后端: auto -> local")
        return generate_image_local(
            prompt=prompt,
            size=size,
            negative_prompt=negative_prompt,
...
class SemanticDialogGenerator:
    ...
    def __init__(
        ...
        # 图像后端通过 IMAGE_GEN_BACKEND / DASHSCOPE_* / LOCAL_IMAGE_* 环境变量控制
        ...
        Note:
            图像生成后端由环境变量控制：
            IMAGE_GEN_BACKEND=dashscope/local/auto
        """
